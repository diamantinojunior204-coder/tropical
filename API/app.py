...
def add_saldo():
    ...
    acao = request.form.get("acao","add")  # add ou remove
    if acao=="remove":
    ...
    def calcular(aposta,c):
        ...
        if grade[0]==grade[1]==grade[2]:
            if grade[0]=="🍒":
                ...
                ganho=aposta+jackpot
                jackpot=100
        # adiciona pequena porcentagem da aposta ao jackpot
        jackpot += aposta*0.03
...
def api_cartas():
    ...
    def calcular(aposta,c):
        ...
        if jogador>dealer:
            ganho=aposta
        return ganho,{"jogador":jogador,"dealer":dealer}
